...
4. **Confidence engine:** `lib/confidence-engine.ts` scores address/program context against a rules matrix, producing program matches with confidence levels (high/medium/low). It is a DISCOVERY engine, not an eligibility engine (see its own header comment). Its optional `survey` parameter (`SurveyAnswers`, the `/check` deep-link `sa=` shape from `lib/url-state.ts` — NOT a wizard field) is currently passed as `undefined` by every live caller; do not wire it back in without an owner ruling. (The standalone `/qualify` questionnaire that produced those answers — `lib/survey-engine.ts`, `components/survey/`, `POST /api/survey/score` — was sunset: owner's ruling was that the product boundary is discovery, not compliance, and that surface had drifted into it.)

...
import { PERMIT_EXHIBIT_COST_LABEL, permitExhibitCityRecordUrl } from "@/lib/permit-exhibit-copy";
import type { PermitExhibitArea, PermitExhibitRow } from "@/lib/permit-exhibit-types";
...
function AreaRecordRow({ row }: { row: PermitExhibitRow }) {
...
function AreaRecordTable({ rows, caption }: { rows: PermitExhibitRow[]; caption: string }) {
  return (
 ...
 * pinned rule), plus the individual records. Rows matched only by address
 * (not confirmed by a geocoded point inside the radius) are disclosed in
 * their own subsection rather than mixed into the point-located table —
 * the same separation discipline S1 applies to proximity rows.
 */
export function AreaContextSection({ area, radiusFt }: { area: PermitExhibitArea; radiusFt: number }) {
  const pointRows = chronological(area.rows.filter((row) => row.matchMethod !== "address_exact"));
  const addressOnlyRows = chronological(area.rows.filter((row) => row.matchMethod === "address_exact"));
  ...
  return (
    <section className="mt-8" aria-labelledby="s2-area-title">
      ...
      <h2 id="s2-area-title" className="mt-1 font-editorial text-[24px] leading-tight text-[#0C1B33]">
        Permits within {radiusFt.toLocaleString("en-US")} ft
      </h2>
      ...
      <div className="mt-4 grid gap-6 border border-[#0C1B33]/10 bg-white p-4 sm:grid-cols-2">
        ...
        />
        <CountBars title="By type" rows={area.byType.map((row) => ({ label: row.type, count: row.count }))} />
      </div>
      ...
      {addressOnlyRows.length > 0 ? (
        <div className="mt-5 border border-[#0C1B33]/15 bg-[#0C1B33]/[0.02] p-4" data-testid="area-address-only-subsection">
          ...
          <p className="mt-2 max-w-2xl text-[12px] leading-relaxed text-[#0C1B33]/55">
            These permits' street address matches a location in this area, but the source record has no
            usable coordinate to confirm the point falls inside the {radiusFt.toLocaleString("en-US")} ft
            radius.
          </p>
          <div className="mt-3">

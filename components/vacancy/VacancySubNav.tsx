 ...
 * area workspace, four views (GPT-5.6 IA consult §3). Server component: the
 * active view is passed in, so no client hooks are needed. The neighborhood
 * switcher preserves the current view when switching ZIPs (reuses the pilot-ZIP
 * chip-row pattern from the admin Owner Files page).
 */

export type VacancyView = "overview" | "areas" | "map" | "directory";
...
export function VacancySubNav({ zip, active }: { zip: string; active: VacancyView }) {
  ...
  return (
    <div className="mb-8 border-b border-[#0C1B33]/10 pb-4">
      ...
      {/* Neighborhood switcher — preserves the active view */}
      ...
      {/* Four-view tabs + Download area PDF */}

...
above, not applied directly). `npm run data:export:investment` was **not**
run as part of this sweep — a reviewer should run it after deciding whether
to accept the proposed megaproject update, per the README's standing
instruction to re-export after curated input changes.

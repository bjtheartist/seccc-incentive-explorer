...
function countActiveZones(zones: Record<string, boolean> | null): number {
  return Object.values(zones ?? {}).filter(Boolean).length;
}

function hasProjectScoping(state: WizardState): boolean {
  return Boolean(
    state.projectType ||
      state.proposedUse ||
      state.budgetRange ||
...
function reportGeneratedEventType(
  reportType: string | null,
  scoped: boolean
): AnalyticsEventType {
  if (reportType === "dev-feasibility" || reportType === "best-location") {
    return "vacancy_report_generated";
  }
  return scoped ? "refined_report_generated" : "location_snapshot_generated";
}
...
function ReportWizardPage() {
    ...
    const timer = setTimeout(() => {
      ...
      try {
        ...
        trackEvent(
          ...
          reportAnalyticsPayload(generated, "instant_report", {
            zoneCount: countActiveZones(zones),
          })
        );
      } catch {
        // Stay on loading
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      try {
        ...
        trackEvent(
          reportGeneratedEventType(generated.reportType, hasProjectScoping(wizardState)),
          reportAnalyticsPayload(generated, "shared_report_open", {
            zoneCount: countActiveZones(zones),
          })
        );
      } catch {
  ...
  const handleGenerateReport = useCallback(async () => {
    ...
    try {
      ...
      trackEvent(
        reportGeneratedEventType(generated.reportType, hasProjectScoping(wizardState)),
        reportAnalyticsPayload(generated, "wizard", {
          zoneCount: countActiveZones(zones),
        })
      );
    } catch {
...
export function ReportDisplay({
  ...
  const handleDownloadAfterCapture = () => {
    ...
    trackEvent(
      ...
      reportAnalyticsPayload(report, "download_gate")
    );
  ...
  const handleVacancySpreadsheetExport = useCallback(async () => {
    ...
    try {
      ...
      trackEvent("spreadsheet_exported", {
        reportType: report.reportType,
        source: "vacancy_report",
        address: vacancySpreadsheetLocale,
        metadata: {
          locale: vacancySpreadsheetLocale,
  ...
  }, [report.reportType, vacancySpreadsheetFeatures, vacancySpreadsheetLocale]);

 ...
 * lines 656-1443. Extracted mechanically (regex over that line range for
 * `const [x, setX] = useState`) rather than by eye — see the maintenance
 * warning above for how to regenerate this if it drifts. Gate review
 * follow-up round 1, MINOR-1: this range (and the two others above/below)
 * was previously self-contradicting AND wrong even on its own terms — the
 * old `~622-1290` upper bound silently excluded the last two real
 * useStates in this function (`corridorAutoGenerated` at L1403,
 * `shareAutoGenerated` at L1443); the array below was correct anyway only
 * because it was hand-verified against the real source, not derived from
 * the stated range.
 *
...
const REPORT_WIZARD_PAGE_STATE_ORDER = [
  ...
  "instantLoading",
  "reverseZip",
  // R1 finding 1 follow-up: `retryNonce` — the monotonic counter that makes
  // the corridor/shared-report "Try again" actually re-run generation (the old
  // `setCorridorAutoGenerated(false)` re-arm was a no-op, because that flag is
  // already false after a failure). It sits HERE, not at the end, because the
  // corridor effect's dep array reads it and so it must be declared above that
  // effect. Every slot below shifts by one; they are seeded by position from
  // this array, so the shift is correct as long as the two move together.
  "retryNonce",
  "corridorAutoGenerated",
  "shareAutoGenerated",
  // R1 finding 1 (the infinite spinner): `reportError` — which
  // generateReportRemote path failed, so /report can render an honest retry
  // card instead of a spinner that never resolves. Declared LAST in
  // `ReportWizardPage`, immediately after `shareAutoGenerated`, precisely so
  // this array only GROWS by one at the end: no existing wizard-page ordinal
  // moves, and every REPORT_DISPLAY_STATE_ORDER slot shifts by exactly one
  // through the FULL_STATE_ORDER concat below, automatically. See the source's
  // own "ORDINAL PLACEMENT" comment in app/report/page.tsx.
  "reportError",
] as const;

/**
 * `useState` call order inside `ReportDisplay`, app/report/page.tsx (continues
 * the SAME shared slot counter, since the mocked `useState` is a single
 * module-level function shared by every component in the render tree —
 * `ReportDisplay` is rendered as a child of `ReportWizardPage` within the
 ...
 * `useVacancySpreadsheetSection` hook (components/report/
 * useVacancySpreadsheetSection.ts), called once from `ReportDisplay` at the
 * exact point these 4 used to sit. React's hook dispatcher — and this
 * file's seeded mock, which is just a shared module-level counter — do not
 * care whether a `useState` call is textually inside the component or
 * inside a custom hook the component calls: the call happens at the same
 * point in the same render pass either way, so the ordinal slot list still
 * has to be regenerated by walking the ACTUAL call sequence, not just this
 * file's own line range. The hook also introduces 4 genuinely NEW slots
 * (`savedAreaPermitAnalysis` through `savedAreaVisibleVacancyCount`) that
 * `ReportDisplay` never had before — parity-by-construction with the
 * area-analysis workstation now reachable from this fork; see
...
const REPORT_DISPLAY_STATE_ORDER = [
  ...
  "isEditingSummary",
  "saveModalOpen",
  // useVacancySpreadsheetSection's own useState order (8 slots) — see the
  // maintenance warning above. Regenerate this whole run if that hook's
  // internal useState order ever changes.
...
function defaultSlotValues(): Record<StateSlotName, unknown> {
  return {
    ...
    savedAreaPermitAnalysis: null,
    savedAreaPermitError: null,
    isLoadingSavedAreaPermit: false,

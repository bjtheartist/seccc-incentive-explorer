...
export default async function VacancyReportPage({
  ...
  // Dollar formatter — negatives are shown as the source publishes them
  // (leading "-$"), never hidden or converted to a derived remainder.
  const fmtTifMoney = (v: number): string =>
    v < 0 ? `-$${Math.abs(Math.round(v)).toLocaleString("en-US")}` : `$${Math.round(v).toLocaleString("en-US")}`;
  // Rule 2 balance-source line: "As of Dec 31, <year> · City of Chicago TIF Annual Report".
  const balanceSourceLine = (reportYear: number): string =>
    `As of Dec 31, ${reportYear} · City of Chicago TIF Annual Report`;
  // Rule 3 scheduled-end: statutory end date + a planning-horizon line, with NO
  // urgency/race phrasing. Replaces the old "Expires this year / Expires in N".
  const scheduledEndLabel = (expirationDate: string | null): string | null => {
    ...
    return `Current scheduled end: ${formatted}`;
  };
  const remainingLine = (yearsLeft: number | null): string => {
    if (yearsLeft == null) return "Scheduled end not on record.";
    if (yearsLeft > 0) return `About ${yearsLeft} ${yearsLeft === 1 ? "year" : "years"} remaining under current law.`;
    if (yearsLeft === 0) return "Scheduled to end this year under current law.";
    return "Past its current scheduled end date under current law.";
  };
        ...
        {tifPicture && tifPicture.length > 0 && (
          <section className="mt-10">
            ...
            <p className="max-w-3xl text-[13px] leading-relaxed text-[#0C1B33]/70">
              ...
              many tracked vacant sites sit inside each. Three separate layers are shown per district and
              never merged: the reported year-end balance (an accounting snapshot), the City&rsquo;s
              programming outlook (a forward-looking plan), and the districts it is near. Present capacity
              is a live City judgment these published figures cannot supply.
            </p>

            {/* Standing explainer (consult §2B) — frames every card below. */}
            <p className="mt-4 max-w-3xl border-l-2 border-[#2563EB]/30 pl-4 text-[13px] leading-relaxed text-[#0C1B33]/70">
              {TIF_STANDING_EXPLAINER}
            </p>

            <div className="mt-5 space-y-px border border-[#0C1B33]/10 bg-[#0C1B33]/10">
              {tifPicture.map((d) => {
                const scheduledEnd = scheduledEndLabel(d.expirationDate);
                return (
                <div key={d.number} className="bg-white px-4 py-4">
                  ...
                  {/* Reported year-end balance (rule 2) + scheduled end (rule 3) */}
                  <div className="mt-3 grid gap-4 sm:grid-cols-2">
                    <div>
                      {d.finance ? (
                        <>
                          <div className="font-editorial text-[26px] leading-none text-[#0C1B33]">
                            {fmtTifMoney(d.finance.fundBalance ?? 0)}
                          </div>
                          <div className="mt-1 font-mono-bureau text-[9px] uppercase tracking-[0.1em] text-[#0C1B33]/45">
                            Reported year-end balance
                          </div>
                          <div className="mt-1 font-mono-bureau text-[10px] text-[#0C1B33]/50">
                            {balanceSourceLine(d.finance.reportYear)}
                          </div>
                          <div className="mt-1 text-[11px] leading-snug text-[#0C1B33]/60">
                            {TIF_BALANCE_CAVEAT}
                          </div>
                        </>
                      ) : (
                        <>
                          <div className="font-editorial text-[15px] leading-none text-[#0C1B33]/45">
                            {TIF_BALANCE_MISSING}
                          </div>
                          <div className="mt-1 font-mono-bureau text-[9px] uppercase tracking-[0.1em] text-[#0C1B33]/45">
                            Reported year-end balance
                    ...
                    <div>
                      <div className="font-mono-bureau text-[10px] uppercase tracking-[0.08em] text-[#0C1B33]/55">
                        {scheduledEnd ?? "Current scheduled end not on record"}
                      </div>
                      <div className="mt-1 text-[11px] leading-snug text-[#0C1B33]/60">
                        {remainingLine(d.yearsToExpiration)}
                      </div>
                    </div>
                  </div>

                  {/* City programming outlook (DPD projections) — each source
                      category on its own row; the two vintages never reconcile. */}
                  <div className="mt-4">
                    <div className="font-mono-bureau text-[9px] uppercase tracking-[0.1em] text-[#0C1B33]/40">
                      City programming outlook
                    </div>
                    {d.projections ? (
                      <>
                        <div className="mt-2 border border-[#0C1B33]/10">
                          {Object.entries(d.projections.categories).map(([category, total]) => (
                            <div
                              key={category}
                              className="flex items-baseline justify-between gap-3 border-b border-[#0C1B33]/5 px-3 py-1.5 last:border-b-0"
                            >
                              <span className="text-[12px] text-[#0C1B33]/75">{category}</span>
                              <span className="flex-shrink-0 font-mono-bureau text-[12px] text-[#0C1B33]">
                                {fmtTifMoney(total)}
                              </span>
                            </div>
                          ))}
                        </div>
                        <div className="mt-1.5 font-mono-bureau text-[9px] uppercase tracking-[0.08em] text-[#0C1B33]/40">
                          DPD 10-Year TIF Projections, published Oct 15, 2025
                        </div>
                        <p className="mt-1.5 max-w-2xl text-[11px] leading-relaxed text-[#0C1B33]/55">
                          {TIF_VINTAGE_NOTE}
                        </p>
                      </>
                    ) : (
                      <p className="mt-1.5 text-[11px] text-[#0C1B33]/45">{TIF_PROJECTIONS_NULL_NOTE}</p>
                    )}
                  </div>

                  {/* Vacancy count (rule 6) — location establishes nothing. */}
                  <div className="mt-4">
                    <div className="flex items-baseline gap-2">
                      <span className="font-editorial text-[22px] leading-none text-[#0C1B33]">
                        {d.vacantSiteCount.toLocaleString("en-US")}
                      </span>
                      <span className="font-mono-bureau text-[9px] uppercase tracking-[0.1em] text-[#0C1B33]/45">
                        Tracked vacant properties within the district
                      </span>
                    </div>
                    <p className="mt-1 text-[11px] leading-snug text-[#0C1B33]/55">
                      {TIF_VACANCY_DISCLAIMER}
                    </p>
                  </div>

                  {/* Neighboring districts (rule 5) — a screening list only. No
                      dollar capacity and no transfer direction are shown. */}
                  <div className="mt-4">
                    <div className="font-mono-bureau text-[9px] uppercase tracking-[0.1em] text-[#0C1B33]/40">
                      {TIF_ADJACENCY_HEADING}
                    </div>
                    {d.neighbors.length === 0 ? (
                      <p className="mt-1.5 text-[11px] text-[#0C1B33]/45">
                        No neighboring district found in the boundary set.
                      </p>
                    ) : (
                      <div className="mt-2 flex flex-wrap gap-2">
                        {d.neighbors.map((n) => (
                          <span
                            key={n.number}
                            className="inline-flex items-baseline gap-1.5 border border-[#0C1B33]/15 bg-[#FAF9F6] px-2 py-1 text-[11px] text-[#0C1B33]/80"
                          >
                            {n.name}
                            <span className="font-mono-bureau text-[9px] text-[#0C1B33]/40">{n.number}</span>
                            <span
                              className="font-mono-bureau text-[8px] uppercase tracking-[0.06em]"
                              style={n.relation === "touching" ? { color: "#0C1B33" } : { color: "#B45309" }}
                            >
            ...
            {/* Adjacency legal framing (consult §2D) + map note — one standing
                block for the whole section. */}
            <p className="mt-4 max-w-3xl text-[12px] leading-relaxed text-[#0C1B33]/60">{TIF_ADJACENCY_BODY}</p>
            <p className="mt-2 max-w-3xl text-[11px] font-semibold leading-relaxed text-[#0C1B33]/60">
              {TIF_ADJACENCY_MAP_NOTE}
            </p>

            {/* DPD-as-authority framing (consult §4) — replaces the old
                who-decides note. DPD confirms; the Explorer only briefs. */}
            <div className="mt-6 border border-[#0C1B33]/15 bg-white px-4 py-4">
              <h3 className="font-mono-bureau text-[11px] font-semibold uppercase tracking-[0.1em] text-[#0C1B33]">
                {TIF_DPD_AUTHORITY_HEADING}
              </h3>
              <p className="mt-2 max-w-3xl text-[12px] leading-relaxed text-[#0C1B33]/70">
                {TIF_DPD_AUTHORITY_BODY}
              </p>
              <p className="mt-3 font-mono-bureau text-[11px] font-semibold uppercase tracking-[0.08em] text-[#2563EB]">
                {TIF_DPD_AUTHORITY_CTA}
              </p>
              <p className="mt-3 max-w-3xl border-t border-[#0C1B33]/10 pt-3 font-mono-bureau text-[10px] uppercase tracking-[0.06em] text-[#0C1B33]/55">
                {TIF_PROCESS_LINE}
              </p>
            ...
            {tifBriefs && (
              <div className="mt-4 text-[11px] leading-relaxed text-[#0C1B33]/45">
                ...
                <ul className="mt-1 list-inside list-disc">
                  ...
                  {tifBriefs.sources.tifProjections && <li>{tifBriefs.sources.tifProjections}</li>}

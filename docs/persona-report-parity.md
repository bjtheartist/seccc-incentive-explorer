...
**Owner ruling — punch-list Q1 (Billy, 2026-08-23): fill the executive
summary to three program names.** The strict persona matches remain the
only visible cards. `personaSummaryProgramNames()` leads with those strict
matches and fills remaining summary slots, in lens order, from the one
collapsed "Also at this address" section. It reads the already-lensed
...
| "Programs matched here" executive-summary cross-link row | **Owner-ratified 2026-08-23 (punch-list Q1):** `components/report/PersonaReportChrome.tsx` renders up to three names via `personaSummaryProgramNames(lensed)`. Strict visible matches lead; if fewer than three exist, remaining slots come in order from the lensed "Also at this address" section. This is a summary-only fill: visible program cards remain strict, and the fill programs remain reachable through the disclosure. Both forks pass the lensed report to the shared component; Looking uses the first three canonical matches because it has no program-card section. | `lib/__tests__/report-personas.test.ts` pins strict-first fill without card promotion; `app/report/__tests__/report-page-live-renderer.test.tsx` pins the three-name surface and forbids non-strict program names outside the summary/disclosure; `lib/__tests__/refine-tier1.test.ts` pins both forks to the shared component. | PASS — owner-ratified Q1 choice |
...
| Program-card anatomy and expansion state | **Corrected 2026-08-23:** on persona boards the first goal-matched card is always expanded with the full `ProgramCardFace` → `ReasonChips` → `ProgramCardExtras` anatomy. Siblings are compact rows with reason chips. The unblessed “Program review details” accordion never renders on a persona board; the All view retains its existing accordion behavior. Consultant-review finding 1 is closed by a per-persona render assertion that every canonical persona section has `data-persona-section-open="true"` and never carries `report-section-collapsed`; mutating the persona `sectionOpen` branch false now fails the suite. | `app/report/__tests__/report-page-live-renderer.test.tsx` asserts visible openness for every persona, exactly one expanded visible face, compact siblings, reason chips, and no persona accordion; component anatomy remains covered by `components/report/__tests__/program-card-order.test.tsx`; `lib/__tests__/refine-tier1.test.ts` pins both forks to the same guarded structure. | PASS after consultant-review correction |
| DOM-level floor suite (gate finding 16, parts a-f) | `app/report/__tests__/report-page-live-renderer.test.tsx` uses genuine `renderToStaticMarkup` assertions against real catalog ids. Consultant-review finding 2 restores the floor's collapse-never-hide behavior: the one compact “Also at this address (N)” line is a native `<details>/<summary>` disclosure containing the excluded program records inline; an excluded name is asserted present inside that disclosure and absent everywhere outside it. `components/report/__tests__/persona-report-chrome.test.tsx` clicks the summary and pins the closed → open → closed interaction plus the revealed program record. The suite also pins the one-line footer, visible reason chips, pinned overlay, explicit empty state, and resolved-share-link gate bypass. | `app/report/__tests__/report-page-live-renderer.test.tsx`; `components/report/__tests__/persona-report-chrome.test.tsx`; `lib/__tests__/shared-link-recipient.test.ts`; fork wiring in `lib/__tests__/refine-tier1.test.ts`. | PASS after consultant-review correction |

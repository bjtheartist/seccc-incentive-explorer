...
| Program-card FACE anatomy (gate finding 11 — administrator, status/window pills, glance row, "Commonly required," next-step/contact, reason chips) | Moved OUT of the collapsed "Program review details" accordion onto the card face, both forks. NEW `components/report/ReasonChips.tsx` (pill-styled `matchExplanation.whyItAppears`, in the engine's existing order — removed from `MatchExplanationDetails`'s own list to avoid printing twice) and NEW `components/report/ProgramCardFace.tsx` (administrator line + status pill from `item.availability`; a glance row of Window + Decision-by; "Commonly required" from `item.eligibilityRules` with a non-suppressible "Your program administrator confirms current eligibility" footer; next-step + contact from the program's own `howToApply[0]`/`contacts[0]`). Four new builders in lib/report-engine.ts (`buildAdministrator`/`buildDecisionBy`/`buildNextStep`/`buildPrimaryContact`), all reading `program.contacts[]`/`howToApply[]` — real, structured, already-published catalog data. **Amount and Type tiles from the R5 board are deliberately NOT built** — no non-blocklisted amount field exists (`Program.benefitRange`'s value is blocked from the canonical report by the same guard gate finding 7 hit) and no structured program-category field exists in the catalog at all; honest omission, not an oversight. `ProgramCardExtras` (worksWith/expectations/verifySources/costSignals) stays inside the accordion for this pass — the finding's explicit element list didn't name it, and moving it too was judged separate, deferrable scope given everything else in this item. | lib/__tests__/program-card-content-fields.test.ts (`buildAdministrator`/`buildDecisionBy`/`buildNextStep`/`buildPrimaryContact` against the real SBIF fixture — multi-contact, real howToApply), components/report/__tests__/reason-chips.test.tsx, components/report/__tests__/program-card-face.test.tsx, components/report/__tests__/executive-summary-section.test.tsx (regression: "Why it appears" no longer double-prints in the accordion), fork-parity in refine-tier1.test.ts | PASS, with `ProgramCardExtras`-stays-in-accordion logged as INTENTIONAL-DIFF (d) |

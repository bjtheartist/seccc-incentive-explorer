...
- [x] Production deployment at https://chicagoincentiveexplorer.com/admin/grants using existing production credentials; anonymous page/API gates verified. See `production-release.md` for remaining live-session checks.
...
The isolated checkout is on `codex/grants-workspace`, based on `origin/main` at `d964651`. The test database is Neon project `winter-hill-01244713`, branch `br-old-salad-aekuxp9b` (`grants-workspace-20260911`). The production release uses the existing production branch `br-rapid-credit-aekjpelt`, with additive migrations and selected working data read back after import. Do not use the test branch as a long-term production database.

The additive, transactional migration is `lib/grants/schema.sql`, invoked with `npm run db:migrate:grants`. It requires the existing `users` table. Run `npm run grants:seed` to insert starter research leads without overwriting staff edits. Starter records do not assert open application rounds. The three selected pilot profiles are stored privately in the production database and are not committed as fixtures.

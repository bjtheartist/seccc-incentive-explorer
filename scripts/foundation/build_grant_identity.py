...
def stable_id(filing, ordinal, fingerprint):
    if filing is None:
        ...
        # filing-bound id.
        return hashlib.sha256(f"unresolved|{fingerprint}|{ordinal}".encode()).hexdigest()[:16]
    ...
    for tag, rows in rows_by_tag.items():
        ...
        for row in rows:
            ...
            out_rows.append({
                ...
                "node_fingerprint": fp,
                "stable_id": stable_id(filing, ordinal, fp),
